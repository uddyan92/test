...
*** Test Cases ***
4.6.9 STC OS UPGRADE CREATE NEW JOB-EXECUTION TYPE PARALLE-BATCH SIZE MAX DEPENDS ON DEVICES 
    ...
    Wait for the loading icon to disappear
    Create device values with active os upgrade status device max parallel verify "${osupgrade.create_new}"
