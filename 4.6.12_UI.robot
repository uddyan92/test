...
*** Test Cases ***
4.6.12 STC OS UPGRADE CREATE NEW JOB-MDT/TCN Mapping Maximum Rows
    ...
    Wait for the loading icon to disappear
    Create device values with active os upgrade status mdt rows "${osupgrade.create_new}"
    ...
    Wait for the loading icon to disappear
    wait until object is clickable via XPath "${BPA__SELECT_MDT_ROWS}"
    click on the object with XPath "${BPA__SELECT_MDT_ROWS}"
    Log to Console   "Extra MDT/Device mapping rows are selected"
